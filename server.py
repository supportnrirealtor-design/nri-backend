...
async def admin_login(credentials: AdminLogin):
    if (
        credentials.username == "NRI.admin"
        and credentials.password == "integrity that truly matters"
    ):
...
async def create_enquiry(enquiry_data: EnquiryCreate):
    ...
    return {
        ...
        "leadrat_synced": leadrat_synced
    }


@api_router.get("/enquiries")
async def get_enquiries():
    enquiries = await db.enquiries.find({}, {"_id": 0}).sort(
        "created_at",
        -1
    ).to_list(1000)
